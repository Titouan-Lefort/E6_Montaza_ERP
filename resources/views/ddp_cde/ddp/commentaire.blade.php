<div class=" p-4 pt-0">
    ...
    <textarea rows="3" id="commentaire" name="commentaire"
        ...
        data-ddp-id="{{ $ddp->id }}" onblur="updateCommentaire(this)">{{ $ddp->commentaire->contenu }}</textarea>
</div>
<script>
     function updateCommentaire(element) {
            const ddpId = element.dataset.ddpId; // Récupère l'ID de la société
            const commentaireTexte = element.value; // Récupère la valeur du commentaire

            // Envoie la requête AJAX avec fetch
            fetch('/ddp/' + ddpId + '/commentaire/save', {
